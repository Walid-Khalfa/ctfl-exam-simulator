...
for (const ex of exams) {
  ...
  // Check counts & links
  console.log(
    `  Questions: ${ex.questions.length}, Options: ${ex.options.length}, Solutions: ${ex.solutions.length}`,
  );
  ...
  for (const s of ex.solutions) {
    ...
    if (!q) {
      console.error(
        `  ❌ Solution references question_id ${s.question_id} which does not exist in questions!`,
      );
    }
  }
