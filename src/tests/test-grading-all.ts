...
// Import all seeds
import { ENSEMBLE_A_EXAM, ENSEMBLE_A_QUESTIONS, ENSEMBLE_A_SOLUTIONS } from "../integrations/supabase/seed-ensemble-a";
import { ENSEMBLE_B_EXAM, ENSEMBLE_B_QUESTIONS, ENSEMBLE_B_SOLUTIONS } from "../integrations/supabase/seed-ensemble-b";
import { ENSEMBLE_C_EXAM, ENSEMBLE_C_QUESTIONS, ENSEMBLE_C_SOLUTIONS } from "../integrations/supabase/seed-ensemble-c";
import { ENSEMBLE_D_EXAM, ENSEMBLE_D_QUESTIONS, ENSEMBLE_D_SOLUTIONS } from "../integrations/supabase/seed-ensemble-d";
import { ENSEMBLE_E_EXAM, ENSEMBLE_E_QUESTIONS, ENSEMBLE_E_SOLUTIONS } from "../integrations/supabase/seed-ensemble-e";
import { ENSEMBLE_F_EXAM, ENSEMBLE_F_QUESTIONS, ENSEMBLE_F_SOLUTIONS } from "../integrations/supabase/seed-ensemble-f";

const exams = [
  { name: "Ensemble A (Exam 1)", exam: ENSEMBLE_A_EXAM, questions: ENSEMBLE_A_QUESTIONS, solutions: ENSEMBLE_A_SOLUTIONS },
  { name: "Ensemble B (Exam 2)", exam: ENSEMBLE_B_EXAM, questions: ENSEMBLE_B_QUESTIONS, solutions: ENSEMBLE_B_SOLUTIONS },
  { name: "Ensemble C (Exam 3)", exam: ENSEMBLE_C_EXAM, questions: ENSEMBLE_C_QUESTIONS, solutions: ENSEMBLE_C_SOLUTIONS },
  { name: "Ensemble D (Exam 4)", exam: ENSEMBLE_D_EXAM, questions: ENSEMBLE_D_QUESTIONS, solutions: ENSEMBLE_D_SOLUTIONS },
  { name: "Ensemble E (Exam 5)", exam: ENSEMBLE_E_EXAM, questions: ENSEMBLE_E_QUESTIONS, solutions: ENSEMBLE_E_SOLUTIONS },
  { name: "Ensemble F (Exam 6)", exam: ENSEMBLE_F_EXAM, questions: ENSEMBLE_F_QUESTIONS, solutions: ENSEMBLE_F_SOLUTIONS },
];
...
async function runGradingTestForAll() {
  ...
  for (const ex of exams) {
    ...
    if (success) {
      ...
      resultsTable.push({ Exam: ex.name, Questions: expectedScore, Score: scoreText, Status: "✅ PASS" });
    } else {
      console.error(`   ❌ Failed! Score is ${scoreText} but expected ${expectedScore}`);
      resultsTable.push({ Exam: ex.name, Questions: expectedScore, Score: scoreText, Status: "❌ FAIL" });
      allPassed = false;
    }
  }
